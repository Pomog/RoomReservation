...
// ReservationSummary displays the reservation summaty page
...
func (m *Repository) AdminCalendarReservations(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		m.App.Session.Put(r.Context(), fmt.Sprintf("blockMap_map_%d", x.ID), blockMap)
	}
...
func (m *Repository) AdminProcessReservations(w http.ResponseWriter, r *http.Request) {
	fmt.Println("in the AdminProcessReservations")
...
func (m *Repository) AdminPostCalendarReservations(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// get the block map from the session. if we have an entry in the map
